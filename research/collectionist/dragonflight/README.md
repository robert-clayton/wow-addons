...
`manifests/achievements.csv` contains the 569 current, player-visible achievements in the seven Dragonflight category trees. `manifests/achievement-criteria.csv` contains their 3,202 current-trackable ordered leaf criteria. The 1,369 hidden category rows remain in the broad inventory for auditing but are not addon entries.
