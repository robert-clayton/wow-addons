...
$manifestRoot = Join-Path $RepoRoot "research\collectionist\battle-for-azeroth\manifests"
...
function Assert-Sequence($actual, $expected, [string]$label) {
    ...
    for ($i = 0; $i -lt $expected.Count; $i++) {
        if ([string]$actual[$i] -ne [string]$expected[$i]) { throw "$label order mismatch at index $($i + 1): expected $($expected[$i]), got $($actual[$i])" }
    }
}
...
function Assert-RareGroups {
    ...
    $raw = Read-Lua "Modules\Rares\Data\BattleForAzeroth.lua"
    $pattern = '(?s)achievementID\s*=\s*(\d+),\s*criteriaCount\s*=\s*(\d+),\s*criteriaTreeIDs\s*=\s*\{([^}]*)\},\s*criteriaNPCIDs\s*=\s*\{([^}]*)\},\s*criteriaObjectIDs\s*=\s*\{([^}]*)\}'
...
function Assert-TreasureGroups {
    ...
    $raw = Read-Lua "Modules\Treasures\Data\BattleForAzeroth.lua"
    $pattern = '(?s)achievementID\s*=\s*(\d+),\s*criteriaCount\s*=\s*(\d+),\s*criteriaTreeIDs\s*=\s*\{([^}]*)\},\s*criteriaNames\s*=\s*\{([^}]*)\}'
    $matches = @([regex]::Matches($raw, $pattern))
    $groups = @($manifest | Group-Object achievement_id)
    ...
    foreach ($match in $matches) {
        ...
        $nameCount = @([regex]::Matches($match.Groups[4].Value, '"(?:\\.|[^"\\])*"')).Count
        if ($nameCount -ne $rows.Count) { throw "treasures achievement $achievementID criteriaNames count mismatch" }
    }
    return [pscustomobject]@{ catalog = "treasures"; rows = $manifest.Count; status = "ordered exact" }
}

$results = @()
$results += Assert-ExactLuaSet "mounts" "Modules\Mounts\Data\BattleForAzeroth.lua" "\bmountID\s*=\s*(\d+)" "mount_id"
$results += Assert-ExactLuaSet "pets" "Modules\Pets\Data\BattleForAzeroth.lua" "\bspeciesID\s*=\s*(\d+)" "species_id"
$results += Assert-ExactLuaSet "toys" "Modules\Toys\Data\BattleForAzeroth.lua" "\bitemID\s*=\s*(\d+)" "item_id"
$results += Assert-ExactLuaSet "decorations" "Modules\Decorations\Data\BattleForAzeroth.lua" "\bdecorID\s*=\s*(\d+)" "decor_id"
$results += Assert-ExactLuaSet "achievements" "Modules\Achievements\Data\BattleForAzeroth.lua" '(?m)^        \{ achievementID\s*=\s*(\d+),\s*name\s*=' "achievement_id"
$results += Assert-ExactLuaSet "recipes" "Modules\Recipes\Data\BattleForAzeroth.lua" '(?m)^        \{ id\s*=\s*(\d+),\s*name\s*=' "recipe_spell_id"
$results += Assert-RareGroups
$results += Assert-TreasureGroups
Assert-UnavailableSet "mounts" "Modules\Mounts\Data\BattleForAzeroth.lua" "mountID" "mount_id"
Assert-UnavailableSet "pets" "Modules\Pets\Data\BattleForAzeroth.lua" "speciesID" "species_id"
Assert-UnavailableSet "toys" "Modules\Toys\Data\BattleForAzeroth.lua" "itemID" "item_id"
...
$achievementLua = Read-Lua "Modules\Achievements\Data\BattleForAzeroth.lua"
...
foreach ($module in @("Mounts", "Pets", "Decorations", "Toys", "Rares", "Treasures", "Achievements", "Recipes")) {
    $path = "Modules\$module\Data\BattleForAzeroth.lua"
    if (@([regex]::Matches($toc, [regex]::Escape($path))).Count -ne 1) { throw "TOC must load $path exactly once" }
}

$results | Format-Table -AutoSize
Write-Host "Collectionist Battle for Azeroth runtime validation passed"
