...
$mountLines.Add("")
$mountLines.Add("-- Wrath of the Lich King mounts. Generated from the exact $($mounts.Count)-row release manifest.")
$mountLines.Add('MC.RegisterContent("wrath", "mounts", {')
...
$mountLines.Add("})")
Write-LuaFile "Modules\Mounts\Data\WrathOfTheLichKing.lua" $mountLines

$pets = Read-Manifest "pets"
$petLines = [System.Collections.Generic.List[string]]::new()
$petLines.Add("local _, MC = ...")
$petLines.Add("")
$petLines.Add("-- Wrath of the Lich King battle pets. Generated from the exact $($pets.Count)-row release manifest.")
$petLines.Add('MC.RegisterContent("wrath", "pets", {')
...
$petLines.Add("})")
Write-LuaFile "Modules\Pets\Data\WrathOfTheLichKing.lua" $petLines

$toys = Read-Manifest "toys"
$toyLines = [System.Collections.Generic.List[string]]::new()
$toyLines.Add("local _, MC = ...")
$toyLines.Add("")
$toyLines.Add("-- Wrath of the Lich King toys. Generated from the exact $($toys.Count)-row release manifest.")
$toyLines.Add('MC.RegisterContent("wrath", "toys", {')
...
$toyLines.Add("})")
Write-LuaFile "Modules\Toys\Data\WrathOfTheLichKing.lua" $toyLines

$decorations = Read-Manifest "decorations"
$decorLines = [System.Collections.Generic.List[string]]::new()
$decorLines.Add("local _, MC = ...")
$decorLines.Add("")
$decorLines.Add("-- Wrath of the Lich King-acquisition housing decor. Ownership follows the awarding content, not the Midnight housing row or visual theme.")
$decorLines.Add('MC.RegisterContent("wrath", "decorations", {')
...
$decorLines.Add("})")
Write-LuaFile "Modules\Decorations\Data\WrathOfTheLichKing.lua" $decorLines
...
$achievementGroups = @{
    "14780" = @("exploration", "zone", "Northrend")
    "14806" = @("features", "dungeons", "Wrath Dungeons")
    "14863" = @("quests", "metas", "Northrend")
    "14866" = @("features", "reputation", "Wrath Reputation")
    "14901" = @("features", "wintergrasp", "Wintergrasp")
    "14922" = @("features", "raid", "Wrath Raids")
    "14941" = @("features", "tournament", "Argent Tournament")
}
$categoryOrder = @("14780", "14806", "14863", "14866", "14901", "14922", "14941")
$achievementLines = [System.Collections.Generic.List[string]]::new()
$achievementLines.Add("local _, MC = ...")
$achievementLines.Add("")
$achievementLines.Add("-- Wrath of the Lich King player-facing achievements. Exact $($achievements.Count)-row manifest;")
$achievementLines.Add("-- stable criteria tasks are attached only for 2-30-row progress lists.")
$achievementLines.Add('MC.RegisterContent("wrath", "achievements", {')
...
$achievementLines.Add("})")
Write-LuaFile "Modules\Achievements\Data\WrathOfTheLichKing.lua" $achievementLines

$recipes = Read-Manifest "recipes"
$recipeLines = [System.Collections.Generic.List[string]]::new()
$recipeLines.Add("local _, MC = ...")
$recipeLines.Add("")
$recipeLines.Add("-- Wrath of the Lich King profession recipes. Generated from the exact $($recipes.Count)-spell manifest.")
$recipeLines.Add('MC.RegisterContent("wrath", "recipes", {')
foreach ($group in $recipes | Group-Object profession | Sort-Object Name) {
    $first = $group.Group[0]
    $recipeLines.Add("    { skillLine = $($first.profession_id), name = `"Wrath of the Lich King`", recipes = {")
...
$recipeLines.Add("})")
Write-LuaFile "Modules\Recipes\Data\WrathOfTheLichKing.lua" $recipeLines

$rareGroups = ,@("2257", "northrend", "Northrend", "MC.MAP.Northrend")
$rares = Read-Manifest "rares"
$rareLines = [System.Collections.Generic.List[string]]::new()
$rareLines.Add("local addonName, MC = ...")
$rareLines.Add("")
$rareLines.Add("-- Wrath of the Lich King zone rares. Exact $($rares.Count) ordered criteria/entity rows.")
$rareLines.Add('MC.RegisterContent("wrath", "rares", {')
...
foreach ($pair in @(@("northrend", "Northrend"))) { $rareLines.Add("    { $(ConvertTo-LuaString $pair[0]), $(ConvertTo-LuaString $pair[1]) },") }
$rareLines.Add("}")
...
$rareLines.Add("end")
Write-LuaFile "Modules\Rares\Data\WrathOfTheLichKing.lua" $rareLines
...
$treasureLines.Add("")
$treasureLines.Add("-- Wrath of the Lich King treasures. Exact $($treasures.Count) ordered criteria rows.")
$treasureLines.Add('MC.RegisterContent("wrath", "treasures", {')
...
$treasureLines.Add("end")
Write-LuaFile "Modules\Treasures\Data\WrathOfTheLichKing.lua" $treasureLines

Write-Host "Generated Wrath of the Lich King Lua data files"
