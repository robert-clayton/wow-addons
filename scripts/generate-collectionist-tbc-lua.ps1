...
$mountLines.Add("")
$mountLines.Add("-- The Burning Crusade mounts. Generated from the exact $($mounts.Count)-row release manifest.")
$mountLines.Add('MC.RegisterContent("tbc", "mounts", {')
...
$mountLines.Add("})")
Write-LuaFile "Modules\Mounts\Data\TheBurningCrusade.lua" $mountLines

$pets = Read-Manifest "pets"
$petLines = [System.Collections.Generic.List[string]]::new()
$petLines.Add("local _, MC = ...")
$petLines.Add("")
$petLines.Add("-- The Burning Crusade battle pets. Generated from the exact $($pets.Count)-row release manifest.")
$petLines.Add('MC.RegisterContent("tbc", "pets", {')
...
$petLines.Add("})")
Write-LuaFile "Modules\Pets\Data\TheBurningCrusade.lua" $petLines

$toys = Read-Manifest "toys"
$toyLines = [System.Collections.Generic.List[string]]::new()
$toyLines.Add("local _, MC = ...")
$toyLines.Add("")
$toyLines.Add("-- The Burning Crusade toys. Generated from the exact $($toys.Count)-row release manifest.")
$toyLines.Add('MC.RegisterContent("tbc", "toys", {')
...
$toyLines.Add("})")
Write-LuaFile "Modules\Toys\Data\TheBurningCrusade.lua" $toyLines

$decorations = Read-Manifest "decorations"
$decorLines = [System.Collections.Generic.List[string]]::new()
$decorLines.Add("local _, MC = ...")
$decorLines.Add("")
$decorLines.Add("-- The Burning Crusade-acquisition housing decor. Ownership follows the awarding content, not the Midnight housing row or visual theme.")
$decorLines.Add('MC.RegisterContent("tbc", "decorations", {')
...
$decorLines.Add("})")
Write-LuaFile "Modules\Decorations\Data\TheBurningCrusade.lua" $decorLines
...
$achievementGroups = @{
    "14777" = @("exploration", "zone", "Eastern Kingdoms")
    "14778" = @("exploration", "zone", "Kalimdor")
    "14779" = @("exploration", "zone", "Outland")
    "14803" = @("features", "eye_of_storm", "Eye of the Storm")
    "14805" = @("features", "dungeons", "The Burning Crusade")
    "14861" = @("quests", "metas", "Eastern Kingdoms")
    "14862" = @("quests", "metas", "Outland")
    "14865" = @("features", "reputation", "TBC Reputation")
    "15081" = @("quests", "metas", "Kalimdor")
}
$categoryOrder = @("14777", "14778", "14779", "14803", "14805", "14861", "14862", "14865", "15081")
$achievementLines = [System.Collections.Generic.List[string]]::new()
$achievementLines.Add("local _, MC = ...")
$achievementLines.Add("")
$achievementLines.Add("-- The Burning Crusade player-facing achievements. Exact $($achievements.Count)-row manifest;")
$achievementLines.Add("-- stable criteria tasks are attached only for 2-30-row progress lists.")
$achievementLines.Add('MC.RegisterContent("tbc", "achievements", {')
...
$achievementLines.Add("})")
Write-LuaFile "Modules\Achievements\Data\TheBurningCrusade.lua" $achievementLines

$recipes = Read-Manifest "recipes"
$recipeLines = [System.Collections.Generic.List[string]]::new()
$recipeLines.Add("local _, MC = ...")
$recipeLines.Add("")
$recipeLines.Add("-- The Burning Crusade profession recipes. Generated from the exact $($recipes.Count)-spell manifest.")
$recipeLines.Add('MC.RegisterContent("tbc", "recipes", {')
foreach ($group in $recipes | Group-Object profession | Sort-Object Name) {
    $first = $group.Group[0]
    $recipeLines.Add("    { skillLine = $($first.profession_id), name = `"The Burning Crusade`", recipes = {")
...
$recipeLines.Add("})")
Write-LuaFile "Modules\Recipes\Data\TheBurningCrusade.lua" $recipeLines

$rareGroups = ,@("1312", "outland", "Outland", "MC.MAP.Outland")
$rares = Read-Manifest "rares"
$rareLines = [System.Collections.Generic.List[string]]::new()
$rareLines.Add("local addonName, MC = ...")
$rareLines.Add("")
$rareLines.Add("-- The Burning Crusade zone rares. Exact $($rares.Count) ordered criteria/entity rows.")
$rareLines.Add('MC.RegisterContent("tbc", "rares", {')
...
foreach ($pair in @(@("outland", "Outland"))) { $rareLines.Add("    { $(ConvertTo-LuaString $pair[0]), $(ConvertTo-LuaString $pair[1]) },") }
$rareLines.Add("}")
...
$rareLines.Add("end")
Write-LuaFile "Modules\Rares\Data\TheBurningCrusade.lua" $rareLines
...
$treasureLines.Add("")
$treasureLines.Add("-- The Burning Crusade treasures. Exact $($treasures.Count) ordered criteria rows.")
$treasureLines.Add('MC.RegisterContent("tbc", "treasures", {')
...
$treasureLines.Add("end")
Write-LuaFile "Modules\Treasures\Data\TheBurningCrusade.lua" $treasureLines

Write-Host "Generated The Burning Crusade Lua data files"
