...
$mountLines.Add("")
$mountLines.Add("-- Warlords of Draenor mounts. Generated from the exact $($mounts.Count)-row release manifest.")
$mountLines.Add('MC.RegisterContent("wod", "mounts", {')
...
$mountLines.Add("})")
Write-LuaFile "Modules\Mounts\Data\WarlordsOfDraenor.lua" $mountLines

$pets = Read-Manifest "pets"
$petLines = [System.Collections.Generic.List[string]]::new()
$petLines.Add("local _, MC = ...")
$petLines.Add("")
$petLines.Add("-- Warlords of Draenor battle pets. Generated from the exact $($pets.Count)-row release manifest.")
$petLines.Add('MC.RegisterContent("wod", "pets", {')
...
$petLines.Add("})")
Write-LuaFile "Modules\Pets\Data\WarlordsOfDraenor.lua" $petLines

$toys = Read-Manifest "toys"
$toyLines = [System.Collections.Generic.List[string]]::new()
$toyLines.Add("local _, MC = ...")
$toyLines.Add("")
$toyLines.Add("-- Warlords of Draenor toys. Generated from the exact $($toys.Count)-row release manifest.")
$toyLines.Add('MC.RegisterContent("wod", "toys", {')
...
$toyLines.Add("})")
Write-LuaFile "Modules\Toys\Data\WarlordsOfDraenor.lua" $toyLines

$decorations = Read-Manifest "decorations"
$decorLines = [System.Collections.Generic.List[string]]::new()
$decorLines.Add("local _, MC = ...")
$decorLines.Add("")
$decorLines.Add("-- Warlords-acquisition housing decor. Ownership follows the awarding content, not the Midnight housing row or visual theme.")
$decorLines.Add('MC.RegisterContent("wod", "decorations", {')
$decorGroups = $decorations | Group-Object {
    if ($_.source_spell_ids -or $_.source_text -match "Profession:|Crafting:") { "crafted" }
...
$decorLines.Add("})")
Write-LuaFile "Modules\Decorations\Data\WarlordsOfDraenor.lua" $decorLines
...
$achievementGroups = @{
    "15220" = @("quests", "metas", "Draenor")
    "15228" = @("features", "dungeons", "Warlords Dungeons")
    "15231" = @("features", "raid", "Warlords Raids")
    "15232" = @("features", "reputation", "Warlords Reputation")
    "15235" = @("exploration", "zone", "Draenor")
    "15237" = @("features", "garrison", "Garrison")
    "15238" = @("features", "buildings", "Garrison Buildings")
    "15239" = @("features", "followers", "Garrison Followers")
    "15240" = @("features", "missions", "Garrison Missions")
    "15241" = @("features", "ashran", "Ashran")
    "15242" = @("features", "monuments", "Garrison Monuments")
    "15249" = @("features", "invasions", "Garrison Invasions")
    "15250" = @("features", "shipyard", "Garrison Shipyard")
}
$categoryOrder = @("15220", "15228", "15231", "15232", "15235", "15237", "15238", "15239", "15240", "15241", "15242", "15249", "15250")
$achievementLines = [System.Collections.Generic.List[string]]::new()
$achievementLines.Add("local _, MC = ...")
$achievementLines.Add("")
$achievementLines.Add("-- Warlords of Draenor player-facing achievements. Exact $($achievements.Count)-row manifest;")
$achievementLines.Add("-- stable criteria tasks are attached only for 2-30-row progress lists.")
$achievementLines.Add('MC.RegisterContent("wod", "achievements", {')
...
$achievementLines.Add("})")
Write-LuaFile "Modules\Achievements\Data\WarlordsOfDraenor.lua" $achievementLines

$recipes = Read-Manifest "recipes"
$recipeLines = [System.Collections.Generic.List[string]]::new()
$recipeLines.Add("local _, MC = ...")
$recipeLines.Add("")
$recipeLines.Add("-- Draenor profession recipes. Generated from the exact $($recipes.Count)-spell manifest.")
$recipeLines.Add('MC.RegisterContent("wod", "recipes", {')
foreach ($group in $recipes | Group-Object profession | Sort-Object Name) {
    $first = $group.Group[0]
    $recipeLines.Add("    { skillLine = $($first.profession_id), name = `"Draenor`", recipes = {")
...
$recipeLines.Add("})")
Write-LuaFile "Modules\Recipes\Data\WarlordsOfDraenor.lua" $recipeLines

$rareGroups = @(
    @("9400", "gorgrond", "Gorgrond", "MC.MAP.Gorgrond"),
    @("10061", "tanaan", "Tanaan Jungle", "MC.MAP.TanaanJungle"),
    @("10070", "tanaan", "Tanaan Jungle", "MC.MAP.TanaanJungle")
)
$rares = Read-Manifest "rares"
$rareLines = [System.Collections.Generic.List[string]]::new()
$rareLines.Add("local addonName, MC = ...")
$rareLines.Add("")
$rareLines.Add("-- Warlords of Draenor zone rares. Exact $($rares.Count) ordered criteria/entity rows.")
$rareLines.Add('MC.RegisterContent("wod", "rares", {')
...
foreach ($pair in @(@("gorgrond", "Gorgrond"), @("tanaan", "Tanaan Jungle"))) { $rareLines.Add("    { $(ConvertTo-LuaString $pair[0]), $(ConvertTo-LuaString $pair[1]) },") }
$rareLines.Add("}")
...
$rareLines.Add("end")
Write-LuaFile "Modules\Rares\Data\WarlordsOfDraenor.lua" $rareLines

$treasureGroups = @(
    @("9548", "nagrand", "Nagrand", "MC.MAP.NagrandDraenor"),
    @("9728", "draenor", "Draenor", "MC.MAP.Draenor"),
    @("10262", "tanaan", "Tanaan Jungle", "MC.MAP.TanaanJungle")
)
...
$treasureLines.Add("")
$treasureLines.Add("-- Warlords of Draenor treasures. Exact $($treasures.Count) ordered criteria rows.")
$treasureLines.Add('MC.RegisterContent("wod", "treasures", {')
...
foreach ($pair in @(@("nagrand", "Nagrand"), @("draenor", "Draenor"), @("tanaan", "Tanaan Jungle"))) { $treasureLines.Add("    { $(ConvertTo-LuaString $pair[0]), $(ConvertTo-LuaString $pair[1]) },") }
$treasureLines.Add("}")
...
$treasureLines.Add("end")
Write-LuaFile "Modules\Treasures\Data\WarlordsOfDraenor.lua" $treasureLines

Write-Host "Generated Warlords of Draenor Lua data files"
